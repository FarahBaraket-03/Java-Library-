...
public class Content1 extends javax.swing.JPanel {
    ...
    public Content1() throws IOException {
      ...
      private void myinitComponents() throws IOException {
        ...
        jTable1.setModel(createtable_dispo(model1,"select * from book where isbn in (select isbn from emprunt where status=0)"));
        
        jTable2.setModel(createtable_dispo(model,"select * from book where qu>0"));
    ...
    //--------------------------------- ADDITION ----------------------------------------------------------------    
        ...
        jLabel3.setText(createCount("select sum(qu) from book"));
        jLabel5.setText(createCount("select count(*) from lecteur"));
        jLabel7.setText(createCount("select count(*) from emprunt where status=1"));
    ...
    public void setTop(JLabel name1,JLabel img1,JLabel name2,JLabel img2,JLabel name3,JLabel img3) throws IOException{
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/bd_bib", "root", "love2PMandGOT7")) 
        {
            String query = "select nom,url from book b , emprunt e where b.isbn=e.isbn group by e.isbn order by (count(cin)) desc";
            try (Statement statement = connection.createStatement();
                 ResultSet rs = statement.executeQuery(query)) {
                if(rs.next()){
                    name1.setText(rs.getString(1));
                    ImageIcon i = new ImageIcon(rs.getString(2));
